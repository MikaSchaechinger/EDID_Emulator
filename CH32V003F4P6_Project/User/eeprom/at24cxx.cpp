...
COMMUNICATION_ERROR AT24CXX::readBytes(uint16_t addr, uint8_t* buffer, uint16_t length)
{
    ...
    while (I2C_GetFlagStatus(I2C1, I2C_FLAG_BUSY) != RESET) {
        if (timeout-- == 0) {
            return BUSY_ERROR;   // Bus is busy for too long
        }
    }
    ...
    if (!this->waitForI2CEvent(I2C_EVENT_MASTER_MODE_SELECT)) {
        return TIMEOUT_ERROR;   // Timeout occurred
    }
    ...
    if (!this->waitForI2CEvent(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
        if (I2C_GetFlagStatus(I2C1, I2C_FLAG_AF) == SET) {  // Check if NACK received   
            ...
            return NACK_ERROR;   // NACK received
        }
        return TIMEOUT_ERROR;   // Timeout occurred
    }
...
COMMUNICATION_ERROR AT24CXX::writeBytes(uint16_t addr, uint8_t* buffer, uint16_t length)
{
    ...
    if (!this->waitForI2CEvent(I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
        if (I2C_GetFlagStatus(I2C1, I2C_FLAG_AF) == SET) {  // Check if NACK received   
            ...
            return NACK_ERROR;   // NACK received, no device responded
        }
        return TIMEOUT_ERROR;   // Timeout occurred
    }
    ...
    return SUCCESS;  // Communication was successful
}
